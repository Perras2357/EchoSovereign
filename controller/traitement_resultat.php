<?php
    ...
    if (isset($_SESSION['titre']) && isset($_POST['resultat']))
    {
        ...
        if (file_exists($voixFile)) 
        {
            ...
            if (array_key_exists($_SESSION['titre'], $voix)) 
            {
                //on vérifie si le vote existe dans le tableau comptage
                if(!empty($voix[$_SESSION['titre']]['comptage']))
                {
                    $response['resultats'] = $voix[$_SESSION['titre']]['comptage'];
